...
在任务1和任务2的基础上，我们添加了一个bonus的任务供大家思考。在`cout`被注释、bug依然存在的情况下，添加一行代码（可以查看`print_bonus.cpp`文件）：

```cpp
if(i == 8 || i == 9) continue;
```

可以通过执行`make bonus`命令编译运行该文件。该任务中你需要回答以下问题：

1. 程序在运行的过程中会有输出吗?
2. 更改之后的程序可以正常运行吗?
    * 如果可以正常运行，解释为什么在bug依然存在的情况下可以正常运行?
    * 如果不可以正常运行，解释此时导致程序崩溃的原因又是什么?
