...
easy_lab2在本仓库的lab2分支上，gitee链接：https://gitee.com/stevenFreeTo/easy_lab

由于lab2分支和lab1分支之间没有共享的commit历史信息，为了避免一些冲突，直接使用以下命令拉取分支，然后就可以在easy_lab2目录下开始做lab了。

```bash
# 将lab2分支拉取到easy_lab2目录下
git clone -b lab2 https://gitee.com/stevenFreeTo/easy_lab.git easy_lab2
# 或者
git clone -b lab2 https://github.com/BUPT-OS/easy_lab.git easy_lab2
```
...
> 由于大家本地机器和评测平台的服务器的配置可能不同，所以在tips部分中给出了服务器的cache的相关信息，可以根据这些信息作为参数设定的参考。
...
提示：数据重排主要是为了将一次运算中需要取出的数据放在连续的内存中。比如矩阵乘法中把乘的矩阵的一列的数据放在一起，避免跨行的跳跃访问。
...
注意：我们将会分配32个核心给你的程序，创建过多的线程可能会导致线程切换的开销过大影响你的程序性能。
